...
namespace msgs
{
    ...
    Message user_register(Server& server, Message& msg)
    {
        ...
        if(msgSize >= MESSAGE_MIN_DATA_SIZE + USER_NAME_SIZE + USER_PASSWD_SIZE * 2)
        {
            ...
            Message response(NULL_CLIENT, MESSAGE_TYPE__UserRegister, 1 + MESSAGE_INFO_MESSAGE_LEN);
            ...
            if (success)
            {
                ...
                server.createUser(msg.getClient(), usrname.data(), usrname.size(), passwd.data(), passwd.size());
            }
    ...
    {
        const User user = server.getUser(msg.getClient());
        if (user == NULL_USER)
        {
            Debug::log(
                "Attempted to update observer for client: " + msg.getClient().getAddress() +
